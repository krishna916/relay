...
const assets: PackageAssets = {
  packageRoot: '/tmp/relay-package',
  migrationsDir: '/tmp/relay-package/assets/migrations',
  webRoot: '/tmp/relay-package/dist/web',
  skillsDir: '/tmp/relay-package/skills',
  integrationsDir: '/tmp/relay-package/integrations',
};
...
describe('doctor package asset check', () => {
  it('reports all executable and immutable assets as healthy', async () => {
    const result = await createPackageAssetsCheck({
      executablePath: '/tmp/relay-package/dist/cli/main.js',
      assets,

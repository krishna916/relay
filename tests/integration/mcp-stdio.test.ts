...
describe('mcp-stdio integration', () => {
  ...
  it('preserves discovery, sessions, duplicates, and protocol operation in the built stdio process', async () => {
    ...
    try {
      ...
      const triaged = (await client.callTool({
        ...
        structuredContent?: { data?: { task?: { status?: string }; change?: { action?: string } } };
      };
      expect(triaged.structuredContent?.data).toMatchObject({
        task: { status: 'ACTIVE' },
        change: { action: 'TRIAGED' },
      });
      ...
      expect(started.structuredContent?.data).toMatchObject({
        task: { status: 'IN_PROGRESS' },
        change: { action: 'STARTED' },
      });
  ...
  it('reports built-process startup failures only on stderr and exits non-zero', async () => {
    const builtJsPath = join(process.cwd(), 'dist', 'mcp', 'main.js');
    const launchDir = mkdtempSync(join(tmpdir(), 'relay-mcp-failure-'));
    const databaseDirectory = join(launchDir, 'database-directory');
    mkdirSync(databaseDirectory);
    try {
      const child = spawn('node', [builtJsPath], {
        cwd: launchDir,
        env: { ...process.env, RELAY_DB_PATH: databaseDirectory },
      ...
      expect(stdout).toBe('');
      expect(stderr).toContain('[ERROR] Fatal error starting MCP stdio server');
    } finally {
      rmSync(launchDir, { recursive: true, force: true });
    }
  });

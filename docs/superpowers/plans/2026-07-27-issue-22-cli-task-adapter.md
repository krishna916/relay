...
expect(stdout).toBe(
  `${JSON.stringify({ schemaVersion: 1, ok: true, data: { task }, warnings: [] })}\n`,
);
...
expect(application.create).toHaveBeenCalledWith(
  expect.objectContaining({ sessionId: 'session-1', creator: { type: 'AGENT', name: 'codex' } }),
);
expect(json(stdout).warnings).toEqual([
  {
...
await expect(
  runCli(['task', 'triage', 'id', '--to', 'DONE', '--output', 'json'], deps),
).resolves.toBe(2);
...
```ts
const result = spawnSync(process.execPath, [builtCliPath, 'task', 'list', '--output', 'json'], {
  cwd: launchDir,

...
| 12. Validation, transition, archived, not-found, and storage errors | `mcp-cli-parity.test.ts` — stable errors and locked-database write failure   | PASS   | Built MCP and CLI writes ran concurrently against one initialized disposable SQLite database while a separate connection held `BEGIN IMMEDIATE`; both returned equal normalized `STORAGE_ERROR` contracts, CLI exit code 5, and sanitized external output |
